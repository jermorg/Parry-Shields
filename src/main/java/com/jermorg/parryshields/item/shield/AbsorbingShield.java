...
public class AbsorbingShield extends ShieldItem {
        ...
        if (isMelee && source instanceof LivingEntity attacker) {
            ...
            if (!weapon.isEmpty()) {
                if (isParrying) {
                    ...
                    attacker.addEffect(new MobEffectInstance(ModEffects.STUN.getHolder().get(), 60));
                    ...
                    if (isPowerfulWeapon) {
                        ...
                        if (!weaponInHand.isEmpty()) {
                            player.getCooldowns().addCooldown(weaponInHand.getItem(), 140);

                            player.level().playSound(null, player.getX(), player.getY(), player.getZ(),
                                    SoundEvents.SHIELD_BLOCK, SoundSource.PLAYERS, 1.0f, 0.8f);
                        }
                    } else {
                        // Вибивання звичайної зброї
                        int count = weapon.getCount();
                        ItemStack dropped = weapon.copy();
                        dropped.setCount(count);
                        attacker.spawnAtLocation(dropped);
    ...
    public void onProjectileImpact(ProjectileImpactEvent event) {
        ...
        if (projectile.getType() == EntityType.TRIDENT) {
            itemCopy = projectile.getWeaponItem().copy();
